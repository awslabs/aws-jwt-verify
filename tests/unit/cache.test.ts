...
describe('unit test SimpleLruCache', () => {
    test('should throw an error if capacity is less than 1', () => {
        expect(() => new SimpleLruCache<number, number>(0)).toThrow('capacity must be greater than 0, but got 0');
    });
    ...
    test('should convert the cache to an array in the correct order', () => {
        ...
        cache.set(1, 100);
        cache.set(2, 200);
        expect(cache.toArray()).toEqual([[1, 100], [2, 200]]);
        cache.get(1);
        cache.set(3, 300);
        expect(cache.toArray()).toEqual([[1, 100], [3, 300]]);
    });
});
