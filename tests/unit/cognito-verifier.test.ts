  ...
  describe("CognitoJwtVerifier", () => {
    ...
    describe("verifySync", () => {
      ...
      test("parseUserPoolId with region format (default)", () => {
        const userPoolId = "us-east-1_123456";
        const { issuer, jwksUri } = CognitoJwtVerifier.parseUserPoolId(userPoolId);
        expect(issuer).toBe("https://cognito-idp.us-east-1.amazonaws.com/us-east-1_123456");
        expect(jwksUri).toBe("https://cognito-idp.us-east-1.amazonaws.com/us-east-1_123456/.well-known/jwks.json");
      });
      
      test("parseUserPoolId with global format from JWT", () => {
        const userPoolId = "us-east-1_123456";
        const jwtIssuer = "https://issuer.cognito-idp.us-east-1.amazonaws.com/us-east-1_123456";
        const { issuer, jwksUri } = CognitoJwtVerifier.parseUserPoolId(userPoolId, jwtIssuer);
        expect(issuer).toBe("https://issuer.cognito-idp.us-east-1.amazonaws.com/us-east-1_123456");
        expect(jwksUri).toBe("https://issuer.cognito-idp.us-east-1.amazonaws.com/us-east-1_123456/.well-known/jwks.json");
      });
      
      test("parseUserPoolId with region format from JWT", () => {
        const userPoolId = "us-east-1_123456";
        const jwtIssuer = "https://cognito-idp.us-east-1.amazonaws.com/us-east-1_123456";
        const { issuer, jwksUri } = CognitoJwtVerifier.parseUserPoolId(userPoolId, jwtIssuer);
        expect(issuer).toBe("https://cognito-idp.us-east-1.amazonaws.com/us-east-1_123456");
        expect(jwksUri).toBe("https://cognito-idp.us-east-1.amazonaws.com/us-east-1_123456/.well-known/jwks.json");
      });
      ...
      test("verify JWT with global issuer format", () => {
        const userPoolId = "us-east-1_123456";
        const globalIssuer = "https://issuer.cognito-idp.us-east-1.amazonaws.com/us-east-1_123456";
        const signedJwt = signJwt(
          { kid: keypair.jwk.kid },
          {
            hello: "world",
        ...
        expect(
          cognitoVerifier.verifySync(signedJwt)
        ).toMatchObject({ hello: "world", iss: globalIssuer });
      });
      
      test("verify JWT with region issuer format", async () => {
        const userPoolId = "us-east-1_123456";
        const regionIssuer = "https://cognito-idp.us-east-1.amazonaws.com/us-east-1_123456";
        const signedJwt = signJwt(
          { kid: keypair.jwk.kid },
          {
            hello: "world",
        ...
        expect(
          await cognitoVerifier.verify(signedJwt)
        ).toMatchObject({ hello: "world", iss: regionIssuer });
      });
    });
  });
